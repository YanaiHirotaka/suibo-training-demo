...
test('避難所は現在の確定位置にある', () => {
  assert.deepEqual(shelter.centerBlock, { x: 25.5, z: 35.5 });
});
...
test('避難所高台は最大水位3.2mより高い', () => {
  ...
  assert.equal(heightAt(25, 35), 13);
});

test('南側の既存道路に沿って坂道が平地まで連続する', () => {
  const samples = [];
  for (let z = 57; z <= 93; z += 3) samples.push(heightAt(27, z));
  assert.equal(samples[0], 13);
  assert.equal(samples.at(-1), 1);
  for (let index = 1; index < samples.length; index += 1) {
    assert.ok(Math.abs(samples[index] - samples[index - 1]) <= 1);
  }
  assert.equal(heightAt(27, 94), 0);
  assert.equal(heightAt(31, 70), 0);
});
